function onReady() {
    ...
    f4_headline.maxLines = 5;
    
    subText.maxLines = 3;
    f2_subText.maxLines = 3;
    f3_subText.maxLines = 3;
    f4_subText.maxLines = 3;
    ...
    function fixFontSize(who) {
        ...
        while (currentHeight > targetHeight) {
            ...
            if (fontSizeS < who.minFontSize) {
                ...
                targetHeight = parseInt(window.getComputedStyle(who).getPropertyValue("line-height"), 10) * who.maxLines;
...
//            ccar.style.height = parseInt(window.getComputedStyle(ctxt).getPropertyValue("line-height"), 10) + 'px';
    ...
    function getTop(who, reference, offset) {
        ...
        if (who != null && reference != null) {
            if (who.innerHTML != "" && who.style.display != "none") {
                who.style.top = Number(window.getComputedStyle(reference).getPropertyValue("top").replace("px", "")) + reference.getBoundingClientRect().height + offset + "px";
            } else {
                who.style.top = Number(window.getComputedStyle(reference).getPropertyValue("top").replace("px", "")) + reference.getBoundingClientRect().height + "px";
            }
        }
    ...
    function getLeft(who, reference, offset) {
        if (who != null && reference != null) {
            who.style.left = Number(window.getComputedStyle(reference).getPropertyValue("left").replace("px", "")) + reference.getBoundingClientRect().width + offset + "px";
        }
    }
